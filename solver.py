...
def Dynamic(input):
    global INF,memo,custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders
    custs=0
    rests=0
    dgs=0
    dishes =0
    ...
    orders = 0
    memo = {}
    custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders =input
    INF = rl*max(cpd)*custs
...
def MIP(input):
    global INF,custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders
    custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders =input
    INF = rl*max(cpd)*custs
    # print("Orders",orders)
...
def getCost(solution):
    ...
    for i in range(len(restaurants)):
        if solution[0][i] >-1:
            obj += (calcualte_dist(delivery_locations[deliverys[i]],restaurant_locations[restaurants[i]])+calcualte_dist(restaurant_locations[restaurants[i]],customer_locations[i]))*cpd[vehicle_types[deliverys[i]]]
    return obj
...
def MetaHeuristic(input):
    global INF,tabu,custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders
    custs,rests,dgs,dishes,rl,vehs,w,vehicle_types,cpd,max_orders,rests_dishes,clients_favs,customer_locations,restaurant_locations,delivery_locations,orders =input
    tabu = []
    INF = rl*max(cpd)*custs
...
if __name__ == '__main__':
    ...
    # for i in range(2,11):
    #     print(generateInput(10,10,10,6,200,5,20,200,i))
